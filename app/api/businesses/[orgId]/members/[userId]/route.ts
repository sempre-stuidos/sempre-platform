...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    console.error('Error in PATCH /api/businesses/[orgId]/members/[userId]:', error);
    return NextResponse.json(
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // (This check should be done in the removeUserFromBusiness function, but adding here for safety)
    if (userId === user.id && role === 'owner') {
      // Check if there are other owners
      // For now, we'll allow it - the business logic should handle this
    }

    const result = await removeUserFromBusiness(orgId, userId);
    ...
    console.error('Error in DELETE /api/businesses/[orgId]/members/[userId]:', error);
    return NextResponse.json(
