...
export function BusinessClients({ orgId, canManage }: BusinessClientsProps) {
  ...
  useEffect(() => {
    const fetchClients = async () => {
      try {
        ...
        const response = await fetch(`/api/businesses/${orgId}/clients`)
        if (!response.ok) {
  ...
  const handleUnlink = async (clientId: number) => {
    if (!confirm('Are you sure you want to unlink this client from the business?')) {
      return
    }
      ...
      {clients.length === 0 ? (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {canManage 
                ? "Link clients to this business to enable client dashboard access."
                : "No clients are linked to this business."}
            </CardDescription>
          </CardHeader>
          {canManage && (
            <CardContent>
