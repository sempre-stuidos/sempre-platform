...
-- RPC function to check user access status, role, and businesses
-- Returns status, role, businesses, and user_id for email/password auth flow
-- ============================================================================
...
BEGIN
  ...
  FROM user_roles
  WHERE invited_email = email_input
  LIMIT 1;
  ...
  IF user_role_record IS NULL THEN
    ...
    WHERE LOWER(au.email) = email_input
    LIMIT 1;
  END IF;
...
-- Add comment
COMMENT ON FUNCTION check_user_access(TEXT) IS 'Checks user access status, role, and businesses for email/password auth flow. Returns status (has_password/needs_password/not_found), role, businesses array, user_id, and user_role_id.';
