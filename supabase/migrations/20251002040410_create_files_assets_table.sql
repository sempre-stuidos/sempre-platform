-- Create files_assets table if it doesn't exist
-- NOTE: This migration is now a no-op since the initial schema already includes files_assets table
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables 
        WHERE table_name = 'files_assets'
    ) THEN
